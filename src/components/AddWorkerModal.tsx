...
interface AddWorkerModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: (data: { name: string; days: string[]; assignedJobs: string }) => void;
}

export const AddWorkerModal = ({ isOpen, onClose, onConfirm }: AddWorkerModalProps) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (name.trim()) {
      onConfirm({ 
        name: name.trim(), 
        days: selectedDays,
        assignedJobs: assignedJobs.trim() || 'Sin asignar'
      });
      setName('');
      setAssignedJobs('');
      setSelectedDays([]);
      ...
      <div className="relative bg-white rounded-t-[40px] p-6 pb-safe-bottom shadow-2xl animate-in slide-in-from-bottom duration-300">
        ...
        <div className="mb-6 pr-12">
          ...
          <p className="text-lg text-slate-500 font-medium">
            Agrega un integrante a tu cuadrilla.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mb-6">
          <div>
            ...
            <input 
              ...
              className="w-full text-2xl font-bold text-slate-900 bg-slate-50 border-4 border-slate-200 rounded-[20px] min-h-[64px] px-6 focus:outline-none focus:border-blue-500 focus:bg-white transition-colors placeholder:text-slate-300"
            />
          </div>
              ...
              {WEEKDAYS.map((day) => {
                ...
                return (
                  <button
                    ...
                    className={`flex-1 min-w-[44px] min-h-[56px] rounded-2xl font-black text-lg transition-colors border-2 ${
                      isSelected 
          ...
          <div>
            ...
            <textarea 
              placeholder="Ej: Mantenimiento Doña Rosa y Familia Gómez"
              value={assignedJobs}
              onChange={(e) => setAssignedJobs(e.target.value)}
              className="w-full text-lg font-bold text-slate-900 bg-slate-50 border-4 border-slate-200 rounded-[20px] min-h-[100px] p-6 focus:outline-none focus:border-blue-500 focus:bg-white transition-colors placeholder:text-slate-300 resize-none"
            />
          </div>
