...
interface AddClientModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: (data: { name: string, basePrice: number }) => void;
}
...
export const AddClientModal = ({ isOpen, onClose, onConfirm }: AddClientModalProps) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (name.trim() && price) {
      onConfirm({ name, basePrice: Number(price) });
      setName('');
      setPrice('');
    }
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mb-6">
          <div>
            ...
            <input 
              ...
              className="w-full text-2xl font-bold text-slate-900 bg-slate-50 border-4 border-slate-200 rounded-[24px] min-h-[72px] px-6 focus:outline-none focus:border-green-500 focus:bg-white transition-colors placeholder:text-slate-300"
            />
          </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input 
                ...
                className="w-full text-3xl font-black text-slate-900 bg-slate-50 border-4 border-slate-200 rounded-[24px] min-h-[72px] pl-14 pr-6 focus:outline-none focus:border-green-500 focus:bg-white transition-colors placeholder:text-slate-300"
              />
            </div>
          </div>
          ...
          <button 
            ...
            className="w-full mt-8 min-h-[80px] bg-blue-600 disabled:bg-slate-300 hover:bg-blue-700 text-white rounded-[28px] text-2xl font-black uppercase tracking-widest shadow-xl shadow-blue-600/30 transition-transform active:scale-95 flex items-center justify-center gap-4"
          >
